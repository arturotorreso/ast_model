...
while(<IN>){
	...
	unless($uniq{$or[1]}){
		...
		my $suoxie_txt_xloci=$flank_x+$width*($xgridN-1)/$xgridN+9;
...
#		foreach my $i (0..7){
		...
		foreach my $i (qw/0 2.5 5 7.5 10/){
...
#			my $x_txt=sprintf("%.1f",1-($i-1)*(1-$miny2{$or[1]})/5);
			...
			#print $x_txt,"\n";
			$svg->text('x',$txt_ygrid_xloci,'y',$txt_ygrid_yloci,'-cdata',$x_txt,'font-family','Arial', 'font-size',$xfontsize+6,'text-anchor','middle','font-weight','bold');#刻度值
		}
	}
...
foreach my $i (1..$xgridN+1){
	...
	my $txt_xgrid_xloci=$flank_x+$i*$grid_len-$grid_len*0.44;
	my $txt_xgrid_yloci=$flank_y*1.5+$height;
	my $x_txt;
	if($i<=$xgridN){
		$x_txt=$xlabel{$i};
...
my $x_lengend_tloci=$flank_y*1.95+$height;
