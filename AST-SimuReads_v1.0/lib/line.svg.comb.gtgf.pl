...
#			my $x_txt=sprintf("%.1f",1-($i-1)*(1-$miny2{$or[1]})/5);
			...
			#print $x_txt,"\n";
			$svg->text('x',$txt_ygrid_xloci,'y',$txt_ygrid_yloci,'-cdata',$x_txt,'font-family','Arial', 'font-size',$xfontsize+3,'text-anchor','middle','font-weight','bold');#刻度值
		}
	}
...
$svg->text('x',$legtxt_line_start_xloci_ppv,'y',$legtxt_line_start_yloci_ppv,'-cdata',"Gene",'font-family','Arial', 'font-size',$xfontsize+3,'font-weight','bold');#刻度值
...
$svg->text('x',$legtxt_line_start_xloci_npv,'y',$legtxt_line_start_yloci_npv,'-cdata',"GeneFamily",'font-family','Arial', 'font-size',$xfontsize+3,'font-weight','bold');#刻度值
...
foreach my $i (1..$xgridN+1){
	...
	my $txt_xgrid_xloci=$flank_x+$i*$grid_len-$grid_len*0.3;
	my $txt_xgrid_yloci=$flank_y*1.45+$height;
	my $x_txt;
	if($i<=$xgridN){
		$x_txt=$xlabel{$i};
