...
@ARGV || die "perl $0 <patho_ARG.detect> > predict.xls\n";
...
my $drug_class_db = "/data/GensKey/Work/hanpeng/01.Product_tech/naiyao/00.pip_ARGdetect/pip_design/bin/../database/drug_class.xls";
...
#print "$report_args\t$scores\n";
#print "$report_args_tmp\t$arg_family_tmp\n";
	        }else{
		    if($argfamily_weight{$ll[0]}{$ll[1]}{$arg_family} > 0){
		        $report_args .= "$arg0;";
...
#print "$arg_family\t$arg0\t$report_args\t$scores\n";
#print "$arg_family\t$argfamily_weight{$ll[0]}{$ll[1]}{$arg_family}\t$arg0\t$arg_weight{$ll[0]}{$ll[1]}{$arg0}\n";
        }
	$report_args ||= "not_detect";
        $report_args =~ s/;$//;
        my $predict_ast = $scores >= $ll[2] ? "R" : $patho_cov{$ll[0]} > 20 ? "S" : "/";
        my $accuracy = $predict_ast ? $ll[3] : "/";
	print "$ll[0]\t$ll[1]\t$report_args\t$scores\t$ll[2]\t$predict_ast\t$accuracy\t$patho_rn{$ll[0]}\t$patho_cov{$ll[0]}\n";
    }
}
close IN;
